...
contract MikeCoinFactory {

    mapping(address => address[]) public created;
    mapping(address => bool) public isMikeCoin; //verify without having to do a bytecode check.
    bytes public mikeCoinByteCode;

    function MikeCoinFactory() {
      //upon creation of the factory, deploy a MikeCoin (parameters are meaningless) and store the bytecode provably.
      address verifiedToken = createMikeCoin(10000, "Verify Token", 3, "VTX");
      mikeCoinByteCode = codeAt(verifiedToken);
    }

    //verifies if a contract that has been deployed is a Human Standard Token.
    //NOTE: This is a very expensive function, and should only be used in an eth_call. ~800k gas
    function verifyMikeCoin(address _tokenContract) constant returns (bool) {
      bytes memory fetchedTokenByteCode = codeAt(_tokenContract);

      if (fetchedTokenByteCode.length != mikeCoinByteCode.length) {
        return false; //clear mismatch
      }

      //starting iterating through it if lengths match
      for (uint i = 0; i < fetchedTokenByteCode.length; i ++) {
        if (fetchedTokenByteCode[i] != mikeCoinByteCode[i]) {
          return false;
        }
      }
